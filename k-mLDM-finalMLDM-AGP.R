...
for(i in 1:K) {
  ...
  res[[i]] <- mLDM(X = Xs, M = Ms, model_selection_num = 1, verbose = FALSE, max_iteration = 5, threshold = 1e-2, max_iteration_B = 100)  
  resi <- res[[i]][[1]]
...
verbose <- TRUE
source("./program/k-Lognormal-Dirichlet-Multinomial-lfbgs-proximal-QUIC-lambda-testZ-AGP.R")
