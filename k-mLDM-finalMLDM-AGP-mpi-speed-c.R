...
dir <- 'simulation-test-new-best-varyCluster-AGP-mpi-speed/'
dir.create(dir)
...
bic <- mclustBIC(M, G = c(K))
model <- Mclust(M, x = bic)
}
...
mldm_init <- foreach(i = 1:K, .packages= c("lbfgs", "QUIC", "psych")) %dopar% {
  ...
  res <- mLDM(X = Xs, M = Ms, model_selection_num = 1, verbose = FALSE, max_iteration = 1, threshold = 1e-2)  
  res
}
timeI1 <- proc.time()
print("mLDM init time:")
print(timeI1 - timeI0)
...
verbose <- TRUE 
sourceCpp("./program/getLogPiProbC.cpp")
source("./program/k-Lognormal-Dirichlet-Multinomial-lfbgs-proximal-QUIC-lambda-testZ-mpi-AGP-speed-c.R")
...
for(j in 1:length2){
  ...
  for(i in 1:length1){
    ...
    if(!exist_best || exist_per[i]){
      ...
      solution <- kmLDM(X, Mscale, K, initParameters, initPi, ZK_init, lambda1, lambda2, max_iteration, threshold, approx_num_Z, max_linesearch_Z, approx_num_B, max_linesearch_B, max_iteration_B, threshold_B, delta1_threshold_B, delta2_threshold_B, sy_threshold_B, max_iteration_B_coor, threshold_B_coor, threshold_lbfgs = 1e-3,loopFlag, verbose)
      print("lambda1:")
      print(solution[[7]])
      print("lambda2:")
      print(solution[[8]])
...
cl <- startMPIcluster(count = K)
registerDoMPI(cl)
mldm_final <- foreach(i in 1:K, .packages=c("lbfgs", "QUIC", "psych")) %dopar% {
