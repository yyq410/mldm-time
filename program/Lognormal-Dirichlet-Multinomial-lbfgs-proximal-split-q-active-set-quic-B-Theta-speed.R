...
if(FALSE) {
  ...
  print(obj)
  print("computelogdet")
  print(computeLogDet(Theta)/2)
  print("s*theta")
  print(sum(diag(S%*%Theta))/2)}
...
LDM <- function(X, M, n, p, q, B, B0, Theta, Z, lambda1, lambda2, max_iteration, threshold, approx_num_Z, max_linesearch_Z, debug, approx_num_B, max_linesearch_B, max_iteration_B, threshold_B, delta1_threshold_B, delta2_threshold_B, sy_threshold_B, max_iteration_B_coor, threshold_B_coor, loopFlag, verbose){
  ...
  while(TRUE){
    ...
    if(loopFlag){
      ...
      # print(min(Z))
        ...
        # check delta of objective function 
        objNew_B <- computeObjf(X, M, B, Theta, Z, lambda1, lambda2)
        delta_B <- abs(objNew_B - objOld_B)
        #print("delta B is:")
        #print(delta_B)
        # iteration B is finished
        if(delta_B < threshold || round >= max_iteration || (exchangeRound < setRound && round_B > maxRoundB)){
            loopFlag <- FALSE
            objNew <- objNew_B 
            if(verbose) {
            print("Round B is:")
            ...
            objList[[round]] <- objNew_B
            next
        }
    }else{
        print("In Theta loop:")
        round_Theta <- round_Theta + 1
        objOld_Theta <- objNew_Theta
        # estimate the Theta
        ...
        print("Theta time:")
        print(timeTheta1 - timeTheta0)
        objNew_Theta <- computeObjf(X, M, B, Theta, Z, lambda1, lambda2)
        delta_Theta <- abs(objNew_Theta - objOld_Theta)
        #print("Delta Theta is:")
        #print(delta_Theta)
        # iteration Theta is finished
        if(delta_Theta < threshold || round >= max_iteration || (exchangeRound < setRound && round_Theta > maxRoundTheta)){
            loopFlag <- TRUE
            objNew <- objNew_Theta
            if(verbose) {
