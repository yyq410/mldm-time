...
kmLDM <- function(X, M, K, initParameters, initPi, Z_init, lambda1, lambda2, max_iteration,
  ...
  while(TRUE) {
    ...
    for(i in 1:n) {
      ...
      for(j in 1:K) {
        ...
        zik_result <- lbfgs(call_eval=objZi, call_grad=derObjZi, vars=zik, mi=mi, xi=xi, 
                         parameters=parameters[[j]], n=n, invisible=1, m=approx_num_Z, max_linesearch=max_linesearch_Z, epsilon = 1e-3, max_iterations = 500)
    ...
    if(needRemove) {
        ...
        derZK <- derZK[-rep(vapply(c(1:n), FUN = function(x) {return(as.integer(((x-1)*K + removeCluster)))}, FUN.VALUE = integer(removeNum))),]
        K <- K - removeNum
        ZK <- ZK[-rep(vapply(removeList, FUN = function(x) {return(as.integer(((x-1)*K + c(1:K))))}, FUN.VALUE = integer(K))),]
        #derZK <- derZK[-rep(vapply(removeList, FUN = function(x) {return(as.integer(((x-1)*K + c(1:K))))}, FUN.VALUE = integer(K))),]
    ...
    if(loopFlag) {
      ...
      for(i in 1:K) {
        ...
        quic_res <- QUIC(S, rho=lambda1_list[i], msg = 0, tol = 1e-3, maxIter = 500)
      ...
      if(deltaTheta < threshold || round >= max_iteration || (roundExchange < setRound && roundTheta >= initRoundTheta)) {
        ...
        print("Theta has been estimated~")
        next
      }
    }
